...
public class ToolServiceImpl implements ToolService {
    ...
    public Mono<ResponseEntity<Object>> doCall(ServerWebExchange exchange,
        ...
        // 2. 获取 McpClient
        McpClient client = mcpClients.get(serviceId);
        if (client == null) {
        ...
        return client.callTool(toolName, arguments)
                .flatMap(result -> {
                    ...
                    // 4. 记录日志
                    recordCallLog(serviceId, exchange, startTime, isError ? 400 : 200, isError ? "Tool execution error" : null);
                    return Mono.just(ResponseEntity.ok().body((Object) result));
                })
                .onErrorResume(e -> {
                    log.error("Tool call failed: service={}, tool={}", serviceId, toolName, e);
                    recordCallLog(serviceId, exchange, startTime, 500, e.getMessage());
     ...
     * 记录工具调用日志
     */
    private void recordCallLog(String serviceId, ServerWebExchange exchange,
