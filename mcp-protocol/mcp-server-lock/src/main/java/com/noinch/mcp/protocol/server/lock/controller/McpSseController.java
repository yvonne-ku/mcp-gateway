package com.noinch.mcp.protocol.server.lock.controller;
...
import com.noinch.mcp.protocol.server.lock.service.LockService;
...
/**
 * MCP SSE 控制器 — Lock Server
 * 手动实现 MCP 2025-11-25 协议
 */
@Slf4j
@RestController
public class McpSseController {
    ...
    public McpSseController(LockService lockService) {
        this.lockService = lockService;
    }

    @PostConstruct
    public void init() {
        toolRegistry.registerTools(lockService, LockService.class);
        log.info("Lock server registered {} tools: {}", toolRegistry.size(), toolRegistry.getToolNames());
    }
        ...
        switch (request.getMethod()) {
            case "initialize" -> {
                Map<String, Object> result = Map.of(
                        ...
                        "serverInfo", Map.of("name", "mcp-server-lock", "version", "1.0.0")
                );
